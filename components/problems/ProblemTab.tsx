...
import { problemData, type Problem, type TrackData, type ProblemData, type Week, type Day } from '@/lib/utils/problemData';
    ...
    <>
      ...
      >
        ...
        <div className="space-y-4">
          {Object.entries(allTracks[selectedTrack]?.weeks || {}).map(([weekKey, rawWeek]) => {
            const weekData = rawWeek as Week;
            const days: Day[] = Array.isArray(weekData.days) ? weekData.days : [];
            const totalProblems = days.reduce(
              (acc, day) => acc + (Array.isArray(day.problems) ? day.problems.length : 0),
              0
            );
                ...
                <div className="relative bg-gray-900/50 backdrop-blur-xl rounded-xl border border-gray-800 overflow-hidden">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-bold text-white">{weekKey}: {weekData?.title ?? ''}</span>
                    </div>
